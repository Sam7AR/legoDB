...
-- 1. Limpiamos variables anteriores
...
-- Nuevas variables para la opción 4
...
ACCEPT v_opcion_menu PROMPT '>> Su elección (1-4): '

PROMPT
-- =========================================================================
-- OPCIÓN 1: VENTA FÍSICA
-- =========================================================================
BEGIN
...
PROMPT
-- =========================================================================
-- OPCIÓN 4: CONFIRMAR PAGO (NUEVA)
-- =========================================================================
BEGIN
    IF &v_opcion_menu = 4 THEN
        DBMS_OUTPUT.PUT_LINE('=== [4] CONFIRMACIÓN DE PAGO DE TOUR ===');
        DBMS_OUTPUT.PUT_LINE('Lista de Recibos actuales:');
    END IF;
END;
/

-- Mostramos la vista bonita que creamos antes
...
WHERE &v_opcion_menu = 4;

PROMPT
PROMPT Ingrese los datos para confirmar el pago:
PROMPT (Nota: La fecha debe ser exacta DD/MM/YY y el número tal cual aparece en la tabla)
ACCEPT v_pago_fecha_str PROMPT '>> Fecha del Tour (DD/MM/YY): ' DEFAULT '01/01/00'
ACCEPT v_pago_nro       PROMPT '>> Nro. Recibo (Solo el número, ej: 2): ' DEFAULT 0


-- =========================================================================
-- BLOQUE PL/SQL DE PROCESAMIENTO
-- =========================================================================
DECLARE
    ...
    -- Variables auxiliares parsing
    v_input_str     VARCHAR2(400);
    v_temp_str      VARCHAR2(100);
    v_comma_pos     NUMBER;
    v_temp_id       NUMBER;

    -- Variables para Opción 4
    v_fecha_pago    DATE;
    
BEGIN
    IF v_opcion NOT IN (1, 2, 3, 4) THEN
    ...
    -- LÓGICA OPCIÓN 4 (NUEVA)
    IF v_opcion = 4 THEN
        v_fecha_pago := TO_DATE('&v_pago_fecha_str', 'DD/MM/YY');
        
        DBMS_OUTPUT.PUT_LINE('Procesando confirmación de pago...');
        DBMS_OUTPUT.PUT_LINE('Fecha Tour: ' || TO_CHAR(v_fecha_pago));
        DBMS_OUTPUT.PUT_LINE('Nro Recibo: ' || &v_pago_nro);
        
        confirmar_pago(
            p_id_tour  => v_fecha_pago,
            p_nro_reci => &v_pago_nro
        );
        
        COMMIT;
        DBMS_OUTPUT.PUT_LINE('>> ¡Pago Confirmado Exitosamente! Entradas generadas.');
    END IF;
