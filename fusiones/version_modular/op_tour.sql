...
PROMPT --- Paso 1: Seleccione Fecha del Tour ---

-- Consulta unificada
SELECT TO_CHAR(t.fec_inic, 'DD/MM/YY') AS fec_inic, (t.cupos_tot - NVL((SELECT COUNT(*) FROM inscritos i WHERE i.id_tour = t.fec_inic), 0)) AS cupos_disp, t.precio_ent FROM tours t WHERE t.fec_inic > SYSDATE;
...
CLEAR SCREEN
PROMPT === [3] INSCRIPCIÓN INSIDE TOUR ===
PROMPT Fecha Seleccionada: &v_tour_fecha_str
PROMPT
...
PROMPT --- Paso 2: Seleccione Adultos (Mayores de 21) ---

-- Consulta unificada
SELECT id_lego, p_nombre || ' ' || p_apellido as cliente_nombre, TRUNC(MONTHS_BETWEEN(SYSDATE, fec_naci)/12) as edad_actual FROM clientes WHERE TRUNC(MONTHS_BETWEEN(SYSDATE, fec_naci)/12) >= 21 ORDER BY id_lego;
...
CLEAR SCREEN
PROMPT === [3] INSCRIPCIÓN INSIDE TOUR ===
PROMPT Fecha: &v_tour_fecha_str | Adultos: &v_tour_cli_str
PROMPT
