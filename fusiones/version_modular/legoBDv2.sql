...
CREATE OR REPLACE FUNCTION obtener_puntos_acumulados(
    p_id_cliente IN NUMBER,
    p_nro_fact   IN NUMBER
) RETURN NUMBER IS
    v_puntos_total NUMBER := 0;
    v_es_gratis    NUMBER := 0;
    v_ult_reset   NUMBER := 0;
BEGIN
    
    SELECT CASE 
             WHEN venta_gratis = 'true' OR venta_gratis = 1 THEN 1 
             ELSE 0 
           END
      INTO v_es_gratis
    ...
    SELECT NVL(MAX(nro_fact), 0)
      INTO v_ult_reset
      FROM facturas_online
     WHERE id_cliente = p_id_cliente
       AND nro_fact < p_nro_fact
       AND (venta_gratis = 'true' OR venta_gratis = 1);
...
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (4, 6, 1);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (4, 6, 2);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (4, 6, 3);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (4, 6, 4);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (2, 6, 5);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (2, 6, 6);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (2, 6, 7);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (10, 6, 9);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (5, 6, 10);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (1, 6, 21);
INSERT INTO CATALOGOS (lim_compra_ol, id_pais, id_juguete) VALUES (1, 6, 24);
...
INSERT INTO clientes VALUES (
111, 'Ionela', 'Cretu', 'Cretu', DATE '1998-07-2', 'E1234567',
6, 6, NULL, NULL, NULL
);
...
SELECT 
    ...
    CASE 
        WHEN pertenece_ue = 'true' OR pertenece_ue = 1 THEN 'Sí'
        ELSE 'No'
    END AS "Miembro U.E."
...
CREATE OR REPLACE VIEW vw_juguetes AS
SELECT
    id,
    nombre "Nombre",
    rgo_edad "Rgo. Edad",
    rgo_precio "Rgo. Precio",
    descripcion "Descripcion",
    CASE 
        WHEN ((es_set = TRUE)) THEN 'Sí'
        ELSE 'No'
    END AS "Set",
    cant_pzas "Cant. piezas",
    id_set_padre "ID Set padre",
    instrucciones "Instrucciones"
FROM juguetes ORDER BY id;
/

CREATE OR REPLACE VIEW vw_facturas_online AS
SELECT 
    f.nro_fact "Nro. factura",
    f.fec_emi "Fecha emisión",
    f.id_cliente "ID Cliente",
    
    CASE 
        WHEN vp."Miembro U.E." = 'Sí' THEN 
            '€' || TO_CHAR(f.costo_tot, 'FM999G999D00')
        ELSE 
            '$' || TO_CHAR(f.costo_tot * 1.08, 'FM999G999D00')
    END AS "Costo Total",

    f.puntos_leal "Puntos de Lealtad",

    obtener_puntos_acumulados(f.id_cliente, f.nro_fact) AS "Puntos Acumulados",
    
    CASE 
        WHEN f.venta_gratis = 'true' OR f.venta_gratis = 1 THEN 'Sí'
        ELSE 'No' 
    END AS "Venta Gratis",
    ...
    CASE 
        WHEN vp."Miembro U.E." = 'Sí' THEN 
            '€' || TO_CHAR(
                CASE 
                    WHEN f.venta_gratis = 'true' OR f.venta_gratis = 1 THEN f.costo_tot
                    ELSE (f.costo_tot / 1.05) * 0.05
                END, 
            'FM999G999D00')
        ELSE 
            '$' || TO_CHAR(
                CASE 
                    WHEN f.venta_gratis = 'true' OR f.venta_gratis = 1 THEN f.costo_tot * 1.08
                    ELSE ((f.costo_tot * 1.08) / 1.15) * 0.15
                END, 
            'FM999G999D00')
    END AS "Recargo de envío",
    
    CASE 
        WHEN vp."Miembro U.E." = 'Sí' THEN 
            '€' || TO_CHAR(
                CASE 
                    WHEN f.venta_gratis = 'true' OR f.venta_gratis = 1 THEN 0
                    ELSE (f.costo_tot / 1.05)
                END, 
            'FM999G999D00')
        ELSE 
            '$' || TO_CHAR(
...
WHERE 
    f.id_cliente = c.id_lego
    AND c.id_pais_resi = vp."ID País";
/

CREATE OR REPLACE VIEW dets_online_con_precio AS
SELECT 
    ...
    CASE 
        WHEN vp."Miembro U.E." = 'Sí' THEN 
            '€ ' || TO_CHAR(h.precio, 'FM999G999D00')
        ELSE 
            '$ ' || TO_CHAR(h.precio * 1.08, 'FM999G999D00')
    END AS "Precio Unitario",
    
    CASE 
        WHEN vp."Miembro U.E." = 'Sí' THEN 
            '€ ' || TO_CHAR(h.precio * d.cantidad, 'FM999G999D00')
        ELSE 
            '$ ' || TO_CHAR((h.precio * 1.08) * d.cantidad, 'FM999G999D00')
    END AS "Precio"
...
WHERE 
    d.nro_fact = f.nro_fact
    AND f.id_cliente = c.id_lego
    AND c.id_pais_resi = vp."ID País"
    AND d.id_juguete_cat = j.id
    AND j.id = h.id_juguete
    AND f.fec_emi >= h.fecha_inicio 
    AND (f.fec_emi <= h.fecha_fin OR h.fecha_fin IS NULL);
/
